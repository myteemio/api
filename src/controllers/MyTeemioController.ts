      ...
      },
      {
        error({ error, set }) {
          ...
          return errorHandler(
            set.status,
            500,
            `Error getting all activities: ${error}`
          );
        },
      ...
      async ({ body, set, params: { id } }) => {
        ...
        if (mongoose.isValidObjectId(id)) {
          ...
          if (body.datesVotedOn.length > 0) {
            ...
            const exists = await dateExistsInTeemio(
              mapMyTeemioToMyTeemioDTO(foundTeemio),
              datesToCheck
            );
            if (!exists) {
              throw new BadRequestError(
                'Date voted on does not exists in Teemio'
              );
            }
          }

          await updateTeemioDateVotesById(
            foundTeemio,
            votingUser,
            body.datesVotedOn
          );
          await updateTeemioActivityVotesById(
            foundTeemio,
            votingUser,
            body.activitiesVotedOn
          );
