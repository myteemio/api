...
export const MyTeemioController = new Elysia({ name: 'routes:myteemio' }).group('/myteemio', (app) => {
    ...
    async ({ body, set }) => {
      try {
        ...
        if (!IsActivityTimeslotsValid(body.activities)) {
          set.status = 400;
          return {
            message: 'Make sure activities begin before they end and that they do not overlap',
  ...
  app.get(
    '/:idorurl',
    ({ params: { idorurl }, set }) => {
      // Create custom return function
      // function Response(errorCode: number, errorMessage?: string)
